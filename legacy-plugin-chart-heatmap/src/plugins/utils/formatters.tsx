import { GenericDataType, NumberFormats, NumberFormatter, TimeFormatter, getNumberFormatter } from "@superset-ui/core";
    ...
    switch (labelType) {
        case EchartsHeatmapLabelTypeType.Key:
            return buildKeyValue(
                color!.toString(),
                getAxisFormated(xAxisDataType, (labelData[0]) as string, timeFormatter, numberFormatter),
                getAxisFormated(yAxisDataType, (labelData[1]) as string, timeFormatter, numberFormatter),)
...
export function axisFormatter(
    params: string,
    timeFormatter: TimeFormatter) {
    return timeFormatter(Number.parseInt(params))
}
export function getAxisFormated(axisDataType: AxisType,
    value: string,
    timeFormatter: TimeFormatter,
    ...
    switch (axisDataType) {
        case AxisType.category:
            return value;
        case AxisType.time:
            return timeFormatter(Number.parseInt(value))
        case AxisType.value:
            return numberFormatter(Number.parseInt(value))
        default:
            return value;
    }
}
